...
class HillClimbing(AlgorithmBase):
    def __init__(self, max_iters=1300, step_size=0.5, num_neighbors=15, step_decay=0.99):
        self.max_iters, self.step_size = max_iters, step_size
        self.num_neighbors, self.step_decay = num_neighbors, step_decay
    ...
    def run(self, problem):
        ...
        for i in range(self.max_iters):
            ...
            # Chỉ cập nhật nếu tìm thấy hàng xóm tốt hơn và không phải là vô cực
            if best_candidate_sol is not None and best_candidate_score < current_score:
                current_sol, current_score = best_candidate_sol, best_candidate_score
                if current_score < best_score:
                    best_sol, best_score = current_sol, current_score

            yield {
                'iteration': i + 1, 'current_solution': current_sol, 'current_score': current_score,
                'best_solution': best_sol, 'best_score': best_score
            }

            if isinstance(problem, ContinuousProblem):
                current_step_size *= self.step_decay
            elif hasattr(problem, 'is_goal') and problem.is_goal(best_sol) and best_score != float('inf'): 
                break
